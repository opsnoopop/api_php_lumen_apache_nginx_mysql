...
export const options = {
  ...
  //     maxVUs: 20000, // สร้าง Virtual Users (VU) สูงสุด 20000 ตัว
  //     stages: [
  ...
  scenarios: {
    contacts: {
      // ใช้ executor แบบ constant-arrival-rate: ยิง request ด้วยความเร็วคงที่
      executor: 'constant-arrival-rate',

      // ระยะเวลาทดสอบ: 1 นาที
      duration: '1m',

      // อัตราการยิง request: 84 requests/วินาที อาจจะต้องหา Magic Number สำหรับทดสอบ
      rate: 1300,

      // หน่วยเวลาของ rate: ต่อ 1 วินาที
      timeUnit: '1s',

      // สร้าง Virtual Users (VU) ล่วงหน้า 200 ตัว
      // เพื่อให้สามารถเริ่มยิง request ได้ทันทีตามอัตราที่กำหนด
      //  อาจจะต้องหา Magic Number สำหรับทดสอบ
      preAllocatedVUs: 3900,
    },
  },
  thresholds: {
...
export function handleSummary(data) {
  ...
  return {
    [filename]: htmlReport(data, {
      title: "health_check_api_php_lumen_fpm_nginx_mysql_" + year + month + day + "_" + hours + minutes + seconds
    }),
  };
}